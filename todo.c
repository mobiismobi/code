...
// تابع ویرایش نام تسک
void edit_task_name() {
    if (selected_task_index < 0 || selected_task_index >= MAX_TASKS) {
...
// تابع ویرایش توضیحات تسک
void edit_task_description() {
    if (selected_task_index < 0 || selected_task_index >= MAX_TASKS) {
        mvprintw(27, 0, "No tasks available to edit.");
        refresh();
        return;
...
// تابع اضافه کردن ددلاین جدید
void add_new_deadline() {
    if (selected_task_index < 0 || selected_task_index >= MAX_TASKS) {
        mvprintw(27, 0, "No tasks available to edit.");
        refresh();
        return;
...
void manage_categories() {
    ...
    while (category_mode) {
        ...
        switch (ch) {
            case 'a':
                if (task_list[selected_task_index].category_count >= MAX_CATEGORIES) {
...
// تابع مرتب‌سازی تسک‌ها بر اساس اولویت
void sort_tasks() {
...
// تابع جستجوی تسک‌ها
void search_tasks(const char *query) {
...
// تابع اضافه کردن زیرتسک جدید
void add_new_subtask() {
    if (task_list[selected_task_index].subtask_count >= MAX_SUBTASKS) {
...
// تابع حذف زیرتسک انتخاب شده
void delete_selected_subtask() {
...
// تابع تغییر وضعیت زیرتسک
void toggle_subtask_status() {
    if (selected_task_index >= 0 && selected_task_index < MAX_TASKS && selected_subtask_index >= 0 && selected_subtask_index < task_list[selected_task_index].subtask_count) {
