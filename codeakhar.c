...
void create_task() { 
    ...
    getnstr(task_title, 49); 

    mvprintw(28, 0, "Enter the number of categories (max %d): ", 10);
    scanw("%d", &tag_count); 
    ...
    for (int i = 0; i < new_task->tag_count; i++) {
        mvprintw(29 + i, 0, "Please enter category %d: ", i + 1);
        getnstr(tag, 29); 
        strncpy(new_task->tags[i], tag, 29);
    }

    do {
        mvprintw(29 + tag_count, 0, "Please enter the deadline (DD/MM/YYYY): ");
        getnstr(due_date, 10);
        if (!check_date_format(due_date)) {
            mvprintw(30 + tag_count, 0, "Invalid date format. Please try again.   ");
        }
    } while (!check_date_format(due_date));

    mvprintw(31 + tag_count, 0, "Please enter the task details: ");
    getnstr(details, 99);

    mvprintw(32 + tag_count, 0, "Please enter the priority (1-9): ");
    ...
    strncpy(new_task->title, task_title, 49); 
...
void manage_tags() { 
    ...
    while (is_tag_mode) {
        ...
        switch (ch) {
            case 'a': 
                if (task_list[current_task_index].tag_count >= 10) { // MAX_CATEGORIES
                    mvprintw(27, 0, "Sorry! Maximum number of categories for this task reached.");
                } else {
                    echo();
                    curs_set(1);
                    ...
                    strncpy(task_list[current_task_index].tags[task_list[current_task_index].tag_count], new_tag, 29);
            ...
            case 'd': 
                if (task_list[current_task_index].tag_count == 0) {
                    ...
                    for (int i = current_category_index; i < task_list[current_task_index].tag_count - 1; i++) {
                        strncpy(task_list[current_task_index].tags[i], task_list[current_task_index].tags[i + 1], 30);
                    }
                    task_list[current_task_index].tag_count--;
...
int compare_by_due_date(const void *a, const void *b) {
    ...
    sscanf(((Task*)a)->due_date, "%d/%d/%d", &day1, &month1, &year1);
    sscanf(((Task*)b)->due_date, "%d/%d/%d", &day2, &month2, &year2);
...
int compare_by_priority_level(const void *a, const void *b) {
    return ((Task*)a)->priority_level - ((Task*)b)->priority_level;
}
...
void sort_task_list() { 
    ...
    switch (sort_choice) {
        case 'n':
            qsort(task_list, total_tasks, sizeof(Task), compare_by_title);
            mvprintw(27, 0, "Tasks sorted by name successfully!                                ");
            break;
        case 'd':
            qsort(task_list, total_tasks, sizeof(Task), compare_by_due_date);
            mvprintw(27, 0, "Tasks sorted by deadline successfully!                           ");
            break;
        case 'p':
            qsort(task_list, total_tasks, sizeof(Task), compare_by_priority_level);
    ...
    cJSON_ArrayForEach(json_task, json_root) { 
        ...
        if (priority && name && description && deadline && categories && subtasks) {
            ...
            cJSON_ArrayForEach(category, categories) {
                if (task_list[total_tasks].tag_count < 10) { // MAX_CATEGORIES
                    strncpy(task_list[total_tasks].tags[task_list[total_tasks].tag_count++], category->valuestring, 29); // CATEGORY_NAME_LENGTH - 1
                }
            }
