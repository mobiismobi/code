...
void bubble_sort_subtasks(Subtask *subtasks, int count) {
    for (int i = 0; i < count - 1; i++) {
        for (int j = 0; j < count - i - 1; j++) {
            if (strcmp(subtasks[j].title, subtasks[j + 1].title) > 0) {
...
void bubble_sort_tags(char **tags, int count) {
    for (int i = 0; i < count - 1; i++) {
        for (int j = 0; j < count - i - 1; j++) {
...
void sort_task_list() { 
    ...
    switch (sort_choice) {
        case 'n':
            bubble_sort_tags(task_list[current_task_index].tags, task_list[current_task_index].tag_count);
            mvprintw(27, 0, "Tags sorted by name successfully!                                ");
            break;
        case 's':
            bubble_sort_subtasks(task_list[current_task_index].sub_items, task_list[current_task_index].sub_item_count);
